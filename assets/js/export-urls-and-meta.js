    ...
    $('#eum-export-form').on('submit', function (event) {
      ...
      activeRequest = $.post(eum_ajax.ajax_url, {
        ...
        .done(function (response) {
          ...
          activeRequestType = null;
          if (!response.success) {
            handleError(response.data && response.data.message ? response.data.message : 'Unable to start export.');
            return;
          }
      ...
      if (sessionId) {
        ...
        $.post(eum_ajax.ajax_url, {
          ...
          .fail(function () {
            ...
            $('#eum-loader-text').text('Unable to cancel export. Try Close again.').css('color', '#b32d2e');
            $('#eum-loader-close').focus();
          });
        return;
      }
    ...
    function processBatch(total, requestToken) {
      ...
      activeRequest = $.post(eum_ajax.ajax_url, {
        ...
        .done(function (response) {
          ...
          activeRequestType = null;
          if (!response.success) {
            handleError(response.data && response.data.message ? response.data.message : 'An error occurred during export.');
            return;
          }

          var processed = parseInt(response.data.processed, 10) || 0;
          var responseTotal = parseInt(response.data.total, 10) || total;
          $('#eum-progress-status').text(processed + ' / ' + responseTotal + ' items processed.');

          if ('complete' === response.data.status) {
            exportInProgress = false;
            $('#eum-loader-text').text('Export complete!');
            $('#eum-download-link')
              .attr('href', eum_ajax.ajax_url + '?action=eum_download_file&nonce=' + encodeURIComponent(eum_ajax.nonce) + '&export_id=' + encodeURIComponent(currentExportId))
              .show();
            resetForm();
            return;
          }
